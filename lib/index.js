...
function handleSplitByLines(filePath, { dest, nbLines, bufferLimit }) {
  return new Promise((resolve, reject) => {
    if (!bufferLimit) bufferLimit = 100000;
    const files = [];
    const baseName = path.parse(filePath).name;
    let curLine = -1;
    let ended = false;
    const queue = async.queue(({ stream, line }, next) => {
      if (queue.length() < bufferLimit * 0.80) lineReader.resume();
      stream.write(`${line}\n`, next)
    });
    let header = null;
    ...
    function getNewFile() {
      ...
      return fs.createWriteStream(path.join(dest || '.', newfilename));
    }
